...
namespace IdentityAPI.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [ActionName("Roles")]
        //[Route("Roles")]
        public async Task<IHttpActionResult> Put([FromBody] JObject json)
        {
         ...
         JToken jid, jroles;
            ...
            if (!json.TryGetValue("id", out jid) || !json.TryGetValue("roles", out jroles))
            {return BadRequest();}

            string id = jid.Value<string>();
            var rolesToAssign = new JArray(jroles);
            IdentityResult removeResult;
            ApplicationUser appUser;
            ///find the user we want to assign roles to
            try
            {
                appUser = await this.UserManager.FindByIdAsync(id);
            ...
            if (appUser == null)
            {
                ...
                var roles = rolesToAssign.ToObject<string[]>();

                var rolesNotExist = roles.Except(RoleManager.Roles.Select(x => x.Name));
                ...
                removeResult = await this.UserManager.RemoveFromRolesAsync(appUser.Id, currentRoles.ToArray());
            ...
            IdentityResult addResult = await this.UserManager.AddToRolesAsync(appUser.Id, roles);
